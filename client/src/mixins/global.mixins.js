...
export default {
	...
	methods: {
		...
		async gSignIn() {
			this.gLoaderShow({});
			try {
				const loginToken = await this.$user.loginToken(this.$bc.accountAddress);
				const signature = await this.$bc.signMessage(loginToken);
				await this.$user.login(this.$bc.accountAddress, signature);
			} catch (error) {
				//console.log(error)
				this.$swal({
					icon: 'error',
		...
		gParseErrorMessage(error) {            
            ...
            try {
                ...
                if (index1 != -1) {                        
					...
					return 'Amplicata HUB: ' + this.$web3.bc.amplicataHub.errorCodes[errorData.args['errorCode']];
                }                    	
            } catch (error) {
